...
export function getSandbox({
	...
	{
		success: { sandbox: Sandbox };
		notFound: {};
		error: { error: any };
	},
	{ id: string }
>) {
	return api.get(`/sandboxes/${props.id}`).then((data) => path.success({ sandbox: data })).catch((error) => {
...
export function optimisticallyAddNpmDependency({ state, props }: Context<{ name: string; version: string }>) {
	const id = state.get('editor.currentId');

	state.set(`editor.sandboxes.${id}.npmDependencies.${props.name}`, props.version);
}
...
export function setUrlOptions({ state, router, utils }) {
	...
	state.set('preferences.showPreview', options.isPreviewScreen || options.isSplitScreen);
	state.set('preferences.showEditor', options.isEditorScreen || options.isSplitScreen);
	...
	if (options.editorSize) state.set('preferences.settings.editorSize', options.editorSize);
	...
	if (options.expandDevTools) state.set('preferences.showConsole', options.expandDevTools);
}
...
export function setCurrentModuleShortid({ utils, props, state }) {
	...
	if (sandbox.modules.map((m) => m.shortid).indexOf(currentModuleShortid) === -1) {
		const module = utils.resolveModule(sandbox.entry, sandbox.modules, sandbox.directories);
...
export function setMainModuleShortid({ utils, props, state }) {
	const module = utils.resolveModule(props.sandbox.entry, props.sandbox.modules, props.sandbox.directories);
...
export function getGitChanges({ api, state }) {
	...
	return api.get(`/sandboxes/${id}/git/diff`).then((gitChanges) => ({ gitChanges }));
}

export function forkSandbox({ state, api }) {
	return api.post(`/sandboxes/${state.get('editor.currentId')}/fork`).then((data) => ({ forkedSandbox: data }));
}
...
export function closeTabByIndex({ state, props }) {
	...
	if (isActiveTab) {
		const newTab = props.tabIndex > 0 ? tabs[props.tabIndex - 1] : tabs[props.tabIndex + 1];

		if (newTab) {
			const newModule = sandbox.modules.find((module) => module.id === newTab.moduleId);
...
export function signInGithub({ browser, path, props }) {
	...
	const popup = browser.openPopup(`/auth/github${useExtraScopes ? '?scope=user:email,public_repo' : ''}`, 'sign in');
...
export function getAuthToken({ api, path }) {
	return api.get('/auth/auth-token').then((token) => path.success({ token })).catch((error) => path.error({ error }));
}
...
export function removeNotification({ state, props }) {
	...
	const notificationToRemoveIndex = notifications.findIndex((notification) => notification.id === props.id);

	state.splice('notifications', notificationToRemoveIndex, 1);
}

export function getUser({ api, path }) {
	return api.get('/users/current').then((data) => path.success({ user: data })).catch(() => path.error());
}
...
export function setPatronPrice({ props, state }) {
	state.set('patron.price', props.user.subscription ? props.user.subscription.amount : 10);
}
