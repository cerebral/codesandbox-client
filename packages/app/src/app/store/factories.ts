import { sequence, Context } from 'fluent';
...
export function addTabById(id) {
	...
	return function addTabById({ state, resolve }) {
		...
		const shortid = modules.find((module) => module.id === resolve.value(id)).shortid;
...
export function setCurrentModuleById(id) {
	...
	return function setCurrentModuleById({ state, resolve }) {
		...
		const module = sandbox.modules.find((moduleEntry) => moduleEntry.id === resolve.value(id));
...
export function setCurrentModule(id) {
	return sequence('setCurrentModule', [ addTabById(id), setCurrentModuleById(id) ]);
}

export function addNotification(title, notificationType, timeAlive = 2, buttons = []) {
	// eslint-disable-next-line
	return function addNotification({ state }: Context) {
		const now = Date.now();
